...
def pytorch_pip(version, requires=None, alias=None):
    ...
    pkg['build_args'] = {
        # retained as $TORCH_CUDA_ARCH_LIST
        'TORCH_CUDA_ARCH_ARGS': ';'.join([f'{x/10:.1f}' for x in CUDA_ARCHITECTURES]),
        'TORCH_VERSION': version,
        'PYTORCH_BUILD_VERSION': build_version,
    }

    if L4T_VERSION.major >= 36:
        # NCCL building only on JP6 and newer
        pkg['build_args']['USE_NCCL'] = 1

    # begin disabling MPI with JP 6.1 since GLOO/NCCL is working
    if Version(version) >= Version('2.5'):
        # and to eliminate security vulnerability from MPI packages
        pkg['build_args']['USE_MPI'] = 0
...
package = [
    ...
    # JetPack 4
    pytorch_wget('1.13', 'torch-1.13.0a0+git7c98e70-cp38-cp38-linux_aarch64.whl',
                 'https://github.com/ShortyGeorge/jetson-nano-ml/releases/download/torch-v1.13.0/torch-1.13.0a0+git7c98e70-cp38-cp38-linux_aarch64.whl', '==32.*'),
    pytorch_wget('1.10', 'torch-1.10.0-cp36-cp36m-linux_aarch64.whl',
                 'https://nvidia.box.com/shared/static/fjtbno0vpo676a25cgvuqc1wty0fkkg6.whl', '==32.*'),
    pytorch_wget('1.9', 'torch-1.9.0-cp36-cp36m-linux_aarch64.whl',
                 'https://nvidia.box.com/shared/static/h1z9sw4bb1ybi0rm3tu8qdj8hs05ljbm.whl', '==32.*'),
]
